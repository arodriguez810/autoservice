        ...
        if (myCrud.table.filters !== undefined) {
            if (myCrud.table.filters.columns === true) {
                ...
                for (var i in myCrud.table.columns) {
                    ...
                    } else if (column.linkfilter) {
                        myCrud.table.filters.columns.push({
                            key: i,
                            type: FILTER.types.relation,
                            label: column.label,
                            table: column.linkfilter.from,
                            value: column.linkfilter.value || "id",
                            text: column.linkfilter.text || "item.nombre",
                            dbFilterColumn: column.linkfilter.whereColumn || column.linkfilter.text.split(".")[1] || column.linkfilter.text,
                            query: {
                                limit: 0,
                                page: 1,
                                where: [],
                                orderby: column.linkfilter.value || "id",
                                order: "asc",
                                distinct: false
                            },
                        });
