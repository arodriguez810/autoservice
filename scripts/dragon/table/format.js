TABLEFORMAT = {
    run: function ($scope) {
        ...
        $scope.tableStatus = function (totalpage) {
            var currentShow =
                $scope.table.currentPage * $scope.table.currentLimit -
                ($scope.table.currentLimit - 1);
            let labeldelsorted = $scope.CRUD()?.columns[$scope.table.orderby]?.label || $scope.table.orderby;
            var result = String.format(
                (totalpage || 2) <= 1 ? MESSAGE.i('table.tableStatusNoPage') : MESSAGE.i('table.tableStatus'),
                $scope.plural,
                MESSAGE.i(`columns.${$scope.table.sortmessage ? $scope.table.sortmessage : $scope.table.orderby}`, labeldelsorted),
