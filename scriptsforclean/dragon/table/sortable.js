SORTABLE = {
    run: function ($scope) {
        ...
        var orderColumn =  (eval(`CRUD_${$scope.modelName}`).table.order || "asc");

        if (eval(`CRUD_${$scope.modelName}`).table.sort)
            firstColumn = eval(`CRUD_${$scope.modelName}`).table.sort;

        if (eval(`CRUD_${$scope.modelName}`).table.order)
            orderColumn = eval(`CRUD_${$scope.modelName}`).table.order;
        ...
        $scope.table.order = orderColumn || "asc";
