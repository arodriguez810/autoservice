...
exports.insertQuery = async function (table, data, params, get, getvalue) {
    ...
    var queries = "";
    console.log(table, "el table insert")
    for (var m in datas) {
        var row = datas[m];
        var columns = [];
        var values = [];
...
exports.update = async function (table, data, params) {
    ...
    var datas = (Array.isArray(data)) ? data : [data];
    var queries = "";
    console.log(table, "el table update")
    for (var m in datas) {
        var data = datas[m];
        var columns = "";
        var values = "";
...
exports.datanoshow = async function (query, params, index) {
                ...
                if (options.join.length > 0) {
                    ...
                    for (var i in options.join) {
                        ...
                        if (obj.table !== undefined) {
                            ...
                            var subwhere = this.makeWhere(obj.where, false);
        ...
        var query = params.format("{sentence} {distinct} {selectfinal} FROM [{table}] " + nickName + " {join} {where} {groupby} {orderby} {order} {limit} {page}",
            {
                ...
                join: join,
                where: where,
                groupby: options.superextra ? "" : groupby,
                orderby: options.superextra ? "" : orderby,
                order: options.superextra ? "" : order,
                limite: options.superextra ? "" : $limit,
                page: options.superextra || $page
            }
        );
            ...
            return exports.data(query, params, {
                ...
                if (options.limit !== undefined)
                    if (data.index !== undefined) {
                        data.totalPage = Math.ceil((groupby ? countData.data.length : countData.data[0].count) / data.index.limitvalue);
                        data.totalCount = (groupby ? countData.data.length : countData.data[0].count);
