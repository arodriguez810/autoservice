            ...
            return users.where(elwheresito).then(async data => {
                ...
                if (data.count[0] > 0) {
                    ...
                    for (var term of params.CONFIG.permissions.terms) {
                        let model_group = new module.Model('group', params);
                        let raw_groups = await model_group.where();
                        var groups = raw_groups.data || [];
                        var user_groups = [];
                        //     await params.storage.getItem(term.relation) || [];
